...
//	window.location.href = BASE_URL+"/goodsRecommend/index"+"?type="+$('#recommendType').val();
	window.location.href = BASE_URL+$('#basePath').val();
}
...
function form_save_added(data, textStatus) {
	if (data.status === 0) {
		...
		if (back_listing == true) {
			...
			window.location.href = BASE_URL+"/goodsRecommend/add";
		}
	} else {
		alert(data.msg);
	}
