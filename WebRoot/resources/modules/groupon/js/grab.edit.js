...
function form_cancel() {
	window.location.href = BASE_URL+"/grouponGrab/index";
}
...
function form_submit() {
	...
	if (! $("#title").val()) {
		notice('edit_notice', img_delete + ' 请输入抢购名称', true, 5000);
		return false;
	}
	
	if (! $("#startTime").val()) {
		notice('edit_notice', img_delete + ' 请选择抢购开始时间', true, 5000);
		return false;
	}
	
	if (! $("#endTime").val()) {
		notice('edit_notice', img_delete + ' 请选择抢购结束时间', true, 5000);
		return false;
	}
	
	if (! $("#inventorySum").val()) {
		notice('edit_notice', img_delete + ' 请输入抢购总库存', true, 5000);
		return false;
	}
	...
	if (! $("#price").val()) {
		notice('edit_notice', img_delete + ' 请输入抢购价格', true, 5000);
		return false;
	}
