...
//			}
	...
	public void outExcelFile(
			List<String> title, List<Object[]> list,
	 ...
	 */
	public void outExcelFile(List<String> title, List<String> title1, List<String> title2,
			List<Object[]> list, String fileName) throws UnsupportedEncodingException{
	    HttpServletResponse response = ((ServletWebRequest)RequestContextHolder.getRequestAttributes()).getResponse();
		response.setContentType("application/vnd.ms-excel;charset=GBK");
		String name = new String((fileName).getBytes("gbk"),"iso8859-1");
		response.setHeader("Content-disposition", "attachment; filename=" + name);
		...
		try {
			os = response.getOutputStream();
			generateExcel(title, title1, title2, list, os);
		} catch (IOException e) {
			e.printStackTrace();
		} 
	}
	
	private void generateExcel(List<String> title1, List<String> title2,
